...
export default function UserOrders() {
  ...
  const pickListFromPayload = (data) => {
  ...
  const fetchOrders = async (opts = {}) => {
    ...
    setLoading(true);
    try {
      // if filtering, load all then slice client-side; otherwise try server pagination
      const url =
        status === "all"
      ...
      const { list } = pickListFromPayload(data);
      ...
      if (status === "all") {
        // prefer server pagination if available
        if (Array.isArray(data?.items) && typeof data?.total === "number") {
          setOrders(data.items);
          setTotal(Number(data.total) || data.items.length);
        } else {
          // fallback: client slice
          const start = p * rpp;
          setOrders(list.slice(start, start + rpp));
          setTotal(list.length);
        }
      } else {
        const filtered = list.filter((o) => String(o.status) === status);
        const start = p * rpp;
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setOpenDialog(false);
      // keep pagination consistent after deletion
